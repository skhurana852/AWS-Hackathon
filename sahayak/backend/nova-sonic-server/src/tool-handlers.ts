...
export async function handleVerifyAadhaar(input: any): Promise<any> {
  ...
  const maskedMobile = `XXXXXX${customer.mobile.slice(-4)}`;
  ...
  if (isRealOtpEnabled()) {
    ...
    return {
      ...
      maskedName: customer.maskedName,
      maskedMobile,
      message: `I've sent a 6-digit OTP to your registered mobile ${maskedMobile}. Please tell me the code once you receive it.`
    };
  }
