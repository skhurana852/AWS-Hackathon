...
/** Returns 'hi' if text contains any Devanagari characters, 'en' otherwise. */
function detectTextLanguage(text: string): 'hi' | 'en' {
  return /[\u0900-\u097F]/.test(text) ? 'hi' : 'en';
}

// Tracks the last detected language per socket session for change detection
const sessionLanguages = new Map<string, 'hi' | 'en'>();
...
io.on('connection', (socket) => {
  ...
  socket.on('textInput', async (text: string) => {
    try {
      ...
      // Detect language and track switches.
      // - Hindi text: send with a brief inline Hinglish cue; also fire a SYSTEM hint.
      //   (Hindi text can never be a pure number, so adding a cue is always safe.)
      // - English/numeric text: send completely verbatim so number extraction is unaffected.
      const lang = detectTextLanguage(text);
      const lastLang = sessionLanguages.get(socket.id) ?? 'en';
      sessionLanguages.set(socket.id, lang);

      if (lang !== lastLang) {
        const hint = lang === 'hi'
          ? 'LANGUAGE SWITCH: Customer switched to Hindi. Respond in casual Hinglish from now on. Do NOT use English.'
          : 'LANGUAGE SWITCH: Customer switched to English. Respond in English from now on. Do NOT use Hindi.';
        session.sendLanguageHint(hint);
        console.log(`[${socket.id}] Language switched: ${lastLang} → ${lang}`);
      }

      // For Hindi input, append a short inline cue so the model sees the
      // language signal right next to the message content.
      // For English/numeric input, never modify — numbers must reach the model verbatim.
      const textToSend = lang === 'hi'
        ? `${text} [Hinglish mein jawab do]`
        : text;

      await session.sendTextInput(textToSend);
    } catch (error) {
