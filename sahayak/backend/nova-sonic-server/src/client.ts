...
export class StreamSession {
  ...
  /** Inject a mid-conversation SYSTEM instruction (e.g. language switch hint). */
  public sendLanguageHint(hint: string): void {
    if (!this.isActive) return;
    this.client.sendSystemTextBlock(this.sessionId, hint);
  }
...
export class NovaSonicBidirectionalStreamClient {
  ...
  // ─── System Text Block (mid-conversation instruction) ───────────────────────

  /** Send a SYSTEM-role text block mid-conversation to update model instructions. */
  public sendSystemTextBlock(sessionId: string, content: string): void {
    const session = this.activeSessions.get(sessionId);
    if (!session || !session.isActive) return;
